...
const errorHandle = (err, req, res, next) => {
  ...
  if (err.name === "DuplicateFieldTrack") {
    const respo = response(409, false, [], err.message);
    console.log(err);
  ...
  if (err.name === "MongoServerError" && err.code === 11000) {
    ...
    const respo = response(
      ...
      false,
      [],
      `Un(e) ${value} est invalide ou existe déjà. Veuillez choisir un autre`
    );
    return res.status(409).json(respo);
  }
  ...
  if (err.name === "JsonWebTokenError") {
    const respo = response(
      ...
      false,
      [],
      "Vous n'avez pas d'autorisation d'accéder à cette ressource"
    );
